...
export default function CreateDebtModal({
  ...
  const { cachedState, saveToCache, clearCache, hasValidCache } =
    useModalCache();
  const {
    calculateUSDValue,
    formatUSDValue,
  ...
  // Handle transaction completion - Don't auto-switch to success step, let popup handle it
  useEffect(() => {
    if (receipt && step === "pending") {
      if (receipt.status === "success") {
        // Don't immediately switch to success step or call callbacks
        // Let TransactionConfirmationPopup handle the countdown and reload
        // When page reloads, data will be automatically fresh
        clearCache();
        // Keep transactionHash for popup display
        // Don't call onPositionCreated yet - let popup countdown handle reload
      } else {
    ...
    try {
      const params: CreatePositionParams = {
        collateralAssets: collateralAssets
          .filter(
            (asset) =>
              asset.selected && asset.amount && parseFloat(asset.amount) > 0
          )
          .map((asset) => {
            const token = tokens[asset.symbol];
            return {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
          ...
          <div className="p-6">
                ...
                <div>
                  ...
                  <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                    {Object.entries(tokens).map(([symbol, token]) => {
                      const assetState = collateralAssets.find(
                        (a) => a.symbol === symbol
                      );
                      return (
                        <div
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-xs text-gray-600 font-medium">
                                  ...
                                  {parseFloat(
                                    tokenBalances[
                                      symbol as keyof typeof tokenBalances
                                    ]?.formatted || "0"
                            ...
                            {assetState?.selected && (
                              <div className="flex items-center space-x-2">
                                <div className="text-right">
                                  ...
                                  />
                                  {assetState.amount &&
                                    parseFloat(assetState.amount) > 0 && (
                ...
                {(() => {
                  ...
                  const totalValue = getTotalUSDValue(
                    collateralAssets
                      .filter(
                        (a) =>
                          a.selected && a.amount && parseFloat(a.amount) > 0
                      )
            ...
            {step === "select-borrow" && (
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-semibold text-gray-800 mb-3">
                    Borrow Assets (Multi-Select)
                  </label>
                  <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                    {Object.entries(tokens).map(([symbol, token]) => {
                      const isCollateral = collateralAssets.find(
                        (a) => a.symbol === symbol && a.selected
                      );
                      const borrowState = borrowAssets.find(
                        (a) => a.symbol === symbol
                      );
                      return (
                        <div
                            ...
                            {borrowState?.selected && !isCollateral && (
                              <div className="flex items-center space-x-3">
                                <div className="text-right">
                                  ...
                                  />
                                  {borrowState.amount &&
                                    parseFloat(borrowState.amount) > 0 && (
              ...
              <div className="space-y-6">
                        ...
                        >
                          ...
                          <div className="text-right">
                            ...
                            <div className="text-xs text-blue-600">
                              {formatUSDValue(
                                calculateUSDValue(asset.amount, asset.symbol)
                              )}
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="flex justify-between pt-2 border-t border-gray-200">
                    ...
                    <span className="font-mono text-sm text-gray-900">
                      {typeof predictedAddress === "string"
                        ? predictedAddress.slice(0, 12) +
                          "..." +
                          predictedAddress.slice(-4)
                        : "Loading..."}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {(() => {
                    ...
                    const allApproved = selectedCollaterals.every((asset) =>
                      approvedTokens.has(asset.symbol)
                    );
                    const hasPendingApprovals = selectedCollaterals.some(
                      (asset) => pendingApprovals.has(asset.symbol)
                    );
                    ...
                    return (
                      <button
                        ...
                        disabled={
                          isApproving || allApproved || hasPendingApprovals
                        }
                  ...
                  <button
                    ...
                    disabled={
                      ...
                      !(() => {
                        ...
                        return selectedCollaterals.every((asset) =>
                          approvedTokens.has(asset.symbol)
                        );
                      })()
                    }
            ...
            {/* Step 4: Pending Transaction - Now handled by TransactionConfirmationPopup */}
              ...
              <div className="text-center space-y-6">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    Your debt position has been successfully created.
                  </p>
                </div>
      ...
      {step === "pending" && (
        <TransactionConfirmationPopup
          isOpen={true}
          onClose={() => {
            // When countdown finishes and popup closes, page will reload
          }}
          onSuccess={() => {
            // This will be called before page reload, after 60s countdown
            onPositionCreated?.();
          }}
          ...
          error={
            receiptError ||
            (transactionError ? new Error(transactionError!) : undefined)
          }
          statusMessage={
            transactionHash
          ...
          description="Creating your debt position with selected collateral and borrow assets..."
          allowClose={true}
        />
      )}
    </div>
